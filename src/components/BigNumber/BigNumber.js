...
const BigNumber: ComponentType<Props> = ({ caption, number, percentageChange, subtext }: Props) => {
  return (
    <Styles.Container>
      <Styles.Caption className="govuk-caption-s">{caption}</Styles.Caption>
      <Styles.Number className="govuk-heading-l">{numeral(number).format('0,0')}</Styles.Number>
